...
//        jaxb : 자바에서 xml 데이터를 자바 objct 타입으로 파싱하도록 도와주는 라이브러리
...
//        실제 xml 데이터 파일을 로드하며 jaxb 라이브러리로 언마샬 실행
//        unmarshal() 메서드의 반환값이 Object 이기 때문에 PharmacyFullDataDto 클래스 타입으로 강제 형변환
//        xml의 모든 데이터를 PharmacyFullDataDto 클래스 타입의 객체에 저장
        PharmacyFullDataDto fullData = (PharmacyFullDataDto) um.unmarshal(new File("C://Java//IntelliJ IDEA//PharmacyFullData.xml"));
        PharmacyHeaderDto headerData = fullData.getHeader();
        PharmacyBodyDto bodyData = fullData.getBody();
        PharmacyItemsDto itmesData = bodyData.getItems();

//        실제 우리가 원하는 데이터만 추출
        List<PharmacyItemDto> itemList = itmesData.getItemList();
